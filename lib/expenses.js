...
var fs = require('fs');
require(path.join(__dirname, '..', 'utils', 'date'));
...
router.get('/expenses', function (req, res) {
    ...
    if (req.query && req.query.searchString) {
        ...
        findObject['$and'].push(filter);
    }

    var date;
    if(req.query && (req.query.createdFrom || req.query.createdTo)) {
        var createdFilter = {};

        if (req.query.createdFrom) {
            date = req.query.createdFrom.toDate();
            if(date) {
                createdFilter['$gt'] = date;
            }
        }

        if (req.query.createdTo) {
            date = req.query.createdTo.toDate();
            if(date) {
                createdFilter['$lt'] = date;
            }
        }
    ...
    if(req.query && (req.query.expenseDateFrom || req.query.expenseDateTo)) {
        var expenseDateFilter = {};

        if (req.query.expenseDateFrom) {
            date = req.query.expenseDateFrom.toDate();
            if(date) {
                expenseDateFilter['$gt'] = date;
            }
        }

        if (req.query.expenseDateTo) {
            date = req.query.expenseDateTo.toDate();
            if(date) {
                expenseDateFilter['$lt'] = date;
            }
        }
    ...
    Expense.find(findObject).count(function (countError, count) {
        ...
        Expense.find(findObject)
            .limit(limit)
            .skip(skip)
            .select('name description created expenseDate amount bill mimeType')
            .sort(sort)
            .exec(function (error, data) {
                if (error) {
                    apiUtils.sendError(res, 400, error);
...
router.post('/expense', uploader.single('file'), function (req, res) {
    ...
    if (!req.body.expenseDate) {
        apiUtils.sendError(res, 400, "Expense date is required");
        return;
    }
